...
from tworaven_apps.datamart_endpoints.forms import DatamartAugmentForm
...
DATAMART_SESSION_TIMEOUT = 60 * 10  # seconds


def make_search_by_dataset_call(
    datamart_name, user_workspace_id, dataset_path, query=None, **kwargs
):
    ...
    kick_off_search_by_dataset.delay(
        datamart_name, user_workspace_id, dataset_path, query=query, **kwargs
    )
...
def kick_off_search_by_dataset(
    datamart_name, user_workspace_id, dataset_path, query=None, **kwargs
):
    """Run this async"""
    SearchUtil(datamart_name, user_workspace_id, dataset_path, query=query, **kwargs)
...
def make_materialize_call(datamart_name, user_workspace_id, datamart_params, **kwargs):
    ...
    kick_off_materialize_steps.delay(
        datamart_name, user_workspace_id, datamart_params, **kwargs
    )
...
@celery_app.task(ignore_result=True)
def kick_off_materialize_steps(
    datamart_name, user_workspace_id, datamart_params, **kwargs
):
    """Run this async"""
    MaterializeUtil(datamart_name, user_workspace_id, datamart_params, **kwargs)


def make_augment_call(user_workspace, augment_params):
...
def kick_off_augment_steps(datamart_name, user_workspace_id, augment_params, **kwargs):
    """Run this async"""
    augment_util = AugmentUtil(
        datamart_name, user_workspace_id, augment_params, **kwargs
    )
