...
class AugmentUtil(BasicErrCheck):
    ...
    def run_augment_steps(self):
        ...
        if self.datamart_name == dm_static.DATAMART_ISI_NAME:
            ...
            self.add_err_msg(f"Materialize not implemented for this datamart type: {self.datamart_name}")
            return
    ...
    def add_err_msg(self, user_msg):
        ...
        user_msg = "%s (datamart augment)" % (user_msg,)

        # ----------------------------------
        # Send Websocket message
        # ----------------------------------
        ws_msg = WebsocketMessage.get_fail_message(
            dm_static.DATAMART_AUGMENT_PROCESS, user_msg
        )
    ...
    def augment_isi_file(self):
        ...
        if not search_result_info.success:
            err_msg = (
                f"Failed to load augment_params['search_result']"
        ...
        keys_to_check = [dm_static.KEY_DATA_PATH, dm_static.KEY_DATASET_DOC_PATH]
        for key in keys_to_check:
            if key not in augment_dict:
                user_msg = (
                    f'Key "{key}" not found in the NYU augment_dict.'
    ...
    def augment_nyu_file(self):
        ...
        for rk in req_keys:
            if not rk in self.augment_params:
                ...
                if not json_info.success:
                    user_msg = (
                        f"Sorry!  Augment failed.  (The data for"
                        f' "{rk}" was not JSON)'
                    )
                    self.add_err_msg(user_msg)
                    return False
        ...
        if keys_not_found:
            user_msg = (
                f"Sorry! Augment failed.  (These required fields"
        ...
        task_data["augmentation"] = {
            ...
            "right_columns": jsonified_data[
                "right_columns"
            ],  # game id in datamart dataset
        }
    ...
    def make_new_dataset(self):
        ...
        new_dataset_util = NewDatasetUtil(
            self.user_workspace.id, self.augment_new_filepath, **extra_params
        )
    ...
    def send_websocket_success_message(self):
        ...
        if not ws_string_info.success:
            user_msg = (
                "Sorry! An error occurred.  (Created workspace but"
