...
class SearchUtil(BasicErrCheck):
    ...
    def __init__(
        self, datamart_name, user_workspace_id, dataset_path, query=None, **kwargs
    ):
    ...
    def send_websocket_success_message(self):
        ...
        resp_info = dict(
            datamart_name=self.datamart_name, search_results=self.search_results
        )
    ...
    def search_with_file(self):
        ...
        user_msg = (
            f"Dataset search not available for datamart: "
    ...
    def run_nyu_search(self):
        """Run the NYU search"""
        params = dict(user_workspace_id=self.user_workspace.id, websocket_id=self.websocket_id)
        search_info = self.datamart_util.search_with_dataset(
            self.dataset_path, query=self.query, **params
        )
    ...
    def add_err_msg(self, user_msg):
        ...
        user_msg = "%s (dataset search)" % (user_msg,)
        ...
        ws_msg = WebsocketMessage.get_fail_message_with_data(
            dm_static.DATAMART_SEARCH_BY_DATASET, user_msg, data=data
        )
