"""
send a gRPC GetFitSolutionResults command
...
def get_fit_solution_results(raven_json_str, user_obj):
    ...
    if not raven_json_str:
        err_msg = 'No data found for the GetFitSolutionResultsRequest'
    ...
    stored_request = StoredRequest(\
                    ...
                    request_type='GetFitSolutionResults',
                    is_finished=False,
    ...
    if settings.TA2_STATIC_TEST_MODE:
        resp_str = get_grpc_test_json(\
                        'test_responses/GetFitSolutionResultsRequest_ok.json',
                        dict())
